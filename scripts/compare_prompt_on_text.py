#!/usr/bin/env python3
"""
Prompt Version Comparison Test - Text File Version
Compares V1 (baseline) vs V2 (enhanced) extraction prompts on plain text
"""
...
class PromptComparison:
    """Compare V1 vs V2 extraction prompts on text"""

    def __init__(self, text_file: Path, provider: str = "openrouter"):
    ...
    def print_sample_events(self, v1_events: List[Dict], v2_events: List[Dict]):
        ...
        print("-" * 80)
        for i, event in enumerate(v1_events[:5], 1):
            print(f"\nEvent {i}:")
        ...
        print("-" * 80)
        for i, event in enumerate(v2_events[:5], 1):
            print(f"\nEvent {i}:")
            print(f"  Date: {event['date'] or '(none)'}")
            particulars = event['event_particulars'][:200] + "..." if len(event['event_particulars']) > 200 else event['event_particulars']
            print(f"  Particulars: {particulars}")
    ...
    def run_comparison(self):
        ...
        print("\n" + "=" * 80)
        print("🧪 PROMPT VERSION COMPARISON TEST (TEXT)")
        print("=" * 80)
